...
async function callNimApi(modelId, payload) {
    ...
    if (!response.ok) {
        ...
        throw new McpError(ErrorCode.InternalError, `NVIDIA NIM API error (${response.status}): ${errorText}`);
    }
    ...
    if (!base64Image) {
        throw new McpError(ErrorCode.InternalError, `Unexpected API response format: ${JSON.stringify(data).substring(0, 500)}`);
    }
    return base64Image;
}
// ─── Image I/O Helpers ──────────────────────────────────────────────
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "generate_image",
            description: "Generate an image from a text prompt using NVIDIA NIM FLUX models. " +
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    cfg_scale: {
                        type: "number",
                        default: 5,
                        description: "Classifier-free guidance scale (1-20). Higher = more faithful to prompt, " +
        ...
        {
            name: "edit_image",
            description: "Edit an existing image using text instructions via FLUX.1-Kontext. " +
            ...
            inputSchema: {
                ...
                properties: {
                    prompt: {
                        type: "string",
                        description: "Text description of the desired edit. Be specific about changes. " +
                            "Example: 'Add a realistic dragon tattoo on the forearm'",
                    },
                    image_path: {
                        type: "string",
                        description: "Absolute or relative path to the source image to edit.",
                    },
                    steps: {
                        type: "number",
                        default: 20,
        ...
        {
            name: "list_models",
            description: "List all available FLUX models on NVIDIA NIM and their capabilities.",
            inputSchema: {
                type: "object",
                properties: {},
            },
    ...
    switch (name) {
        ...
        case "generate_image": {
            ...
            if (modelInfo.type !== "text-to-image") {
                throw new McpError(ErrorCode.InvalidParams, `Model '${model}' is for ${modelInfo.type}, not text-to-image. Use 'flux-1-dev' or 'flux-1-schnell'.`);
            }
        ...
        case "edit_image": {
            ...
            const image_path = rawArgs.image_path;
            const steps = rawArgs.steps ?? 20;
            const cfg_scale = rawArgs.cfg_scale ?? 7;
            const seed = rawArgs.seed ?? 0;
            if (!image_path) {
            ...
            try {
                ...
                throw new McpError(ErrorCode.InvalidParams, `Could not read image at '${image_path}'. Ensure the file exists and is accessible.`);
            }
            const payload = {
                prompt,
                image: imageBase64,
